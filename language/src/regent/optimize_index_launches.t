...
local function optimize_loop_body(cx, node, report_pass, report_fail)
  ...
  for i = 1, #node.block.stats - 1 do
    ...
    if not std.analyze_is_side_effect_free(loop_cx, stat) then
  ...
  if reduce_lhs then
    ...
    if not std.analyze_is_side_effect_free(loop_cx, reduce_lhs) then
  ...
  if not call:is(ast.typed.expr.Call) then
    if call:is(ast.typed.expr.Fill) then
      ...
      if not std.analyze_is_side_effect_free(loop_cx, call.value) then
    ...
    for i, arg in ipairs(args) do
      if not std.analyze_is_side_effect_free(loop_cx, arg) then
